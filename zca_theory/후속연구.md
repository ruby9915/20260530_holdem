...
flowchart TD
    ...
    VIC --> POT["✅ pot-스케일 VIC (30번 실험): OOD 재현"]
    POT --> POTR["✅ off 0/6 → α35% 6/6양 (+1396, seed sweep, plateau 30-50%)"]
    POT --> N1["💡 메커니즘 규명: 왜 이기나 (집계 흡수율 무반응 → +EV 체크 복원?)"]
    POT --> N2["💡 다중 OOD: Random 외 페르소나 일반화"]
    ...
    class R root;
    class DIAG,VIC,METH,PAPER,PROOF,FAM,MEAS,THR,POT,POTR done;
    class N1,N2 idea;
    class MULTI,POS,X3,PARK parked;
    class X1,X2 dead;
```

> **다음 세션 시작점:** 🔵 활성은 비워둠 — `N1(메커니즘 규명)` / `N2(다중 OOD)` 중 하나를 골라 🔵로 바꾸고 시작.
