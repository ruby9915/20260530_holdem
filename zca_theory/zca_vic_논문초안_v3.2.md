...
**(2) 한계.** ① 임계의 정확한 위치는 미확정 — 전이구간(팟 8~15%·상수 1칩 부근)은 seed 요동이 크며(1칩 6-seed 평균 −117·2/6 양수), "충분 초과 시 전 seed 유효"라는 임계의 존재만 주장한다. 아울러 이 임계는 흡수 해소의 임계이며, 성능 회복 문턱과는 분리될 수 있다(5.3.2절). ② 효과의 범위는 단일 상대 학습·단일 게임(헤즈업 노리밋 홀덤)·단일 추상화에 한정된다. ③ 홀드아웃 상대도 자체 제작 규칙 기반 정책이다. 외부·균형 상대 검증은 이후 수행되었다 — CFR+ 동결 전략(추상 내 착취가능성 3.91 mbb/g)을 학습 상대로 한 후속 실험에서, 처방을 적용해도 균형 상대 자체에 대한 성능은 전 조건 음수(표 기반 표현력의 이론적 한계 — 균형 상대 기댓값 상한 ≈0)였으나, 무작위 상대 홀드아웃에서의 선택적 회복은 재현되었다(부정적 결과 포함). ④ ZCA의 부품(null-player 공리·return-equivalence·낙관적 초기화·PBRS≡Q-init)은 이미 알려진 것이며, 기여는 이들을 실패 모드로 재진단하고 임계를 유도·실증하며 범위를 정직히 규정한 데 있다.
