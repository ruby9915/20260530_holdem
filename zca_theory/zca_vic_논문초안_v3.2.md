...
IV장의 이론적 기각(무정보 처방은 고정점의 순위를 바꾸지 못함)을 동일 조건 통제 비교로 실증하였다(처방별 학습 seed 5개, 나머지 설정은 V장 서두와 동일). 비교 대상은 ZCA에 대해 제안될 법한 세 처방이다: ① **탐색 강화**(softmax 온도 하한 2.0), ② **낙관적 초기화**(전 셀 Q₀=+50 — Proposition 1의 검증), ③ **일률 가산 벌점**(전 행동의 credit에서 5칩 차감 — action-penalty[8]류의 비선택적 비용).
