...
**탈출 메커니즘 비교.** 무정보 처방은 이 순위 왜곡을 풀지 못한다 — 결정 시 잡음은 고정점 0을 바꾸지 못하고, 동률 처리(tie-break)는 엄격한(strict) 부등식(0 > 음수)에서는 발동하지 않는다. 가상비용과 고정 벌점만 고정점을 옮기며, 가상비용의 고정점만 참값 μ_C에 연동(informed)된다.
