...
**Theorem′ (은폐 — 좋은 체크의 과소평가).** μ_C > μ_B > 0이면 비례배분 greedy는 Q_prop(CHECK)=0 < [b/(b+c)]μ_B이므로 참-최선인 CHECK를 버리고 열등한 +EV BET을 선택한다(강한 핸드를 숨기는 트랩, 팟 크기를 억제하는 팟 컨트롤 등 체크가 참-최선인 경로의 상실). 즉 ZCA는 특정 부호 조건의 문제가 아니라 **CHECK의 참값이 0이 아닌 모든 결정 상태에서의 양방향 순위 왜곡**이다.
