...
> 목적: "비례배분(proportional) 기여도 배분 하에서 비용 0 행동의 MC 고정점은 진짜값이 아니라 0이고,
> 이 0이 열등 행동을 greedy 정책에서 *영구히* 흡수한다(ZCA). 이는 optimistic initialization의
...
$a_1$ 의 **진짜값**은 $q^\*(s,a_1)=\mu_{a_1}$. 에피소드 총투자액은 $I = \mathrm{inv}(a_1) + c$ (toy에서 $c$ 고정).
...
> 즉 표준 MC는 모든 행동의 Q를 **진짜값**으로 수렴시킨다.
...
> **핵심**: CHECK의 고정점은 진짜값 $\mu_{\text{CHECK}}$ 과 **무관하게 0** 이다 — 구조적 편향. 방문 횟수와
...
> **흡수의 scope (중요).** 흡수는 위 조건 — *덜 나쁜 대안조차 −EV* — 일 때만 발생한다. 어떤 행동이 +EV이면
> $Q_{\text{prop}}>0=Q_{\text{prop}}(\text{CHECK})$ 라 정상 선택되므로, ZCA가 손상시키는 것은 "**모든 가용 행동이
> −EV인 노드에서 *덜 나쁜 것을 고르는 능력***"뿐이다(어떤 행동이라도 +EV면 무해). 이 −EV 제약이 본 정리의
> 가장 중요한 적용 한계다.
>
...
## 6. Proposition (optimistic initialization과의 구분)
...
최적 행동이 선택된다. 즉 optimistic init의 흡수는 *수렴 전 일시적*이고 **고정점은 진짜값**이다.

대조적으로 비례배분에서 $Q_{\text{prop}}(\text{CHECK})=0$ 은 *수렴 후에도 유지되는 고정점*이며 진짜값 $\mu_C$ 과
다르다(Lemma 2). 따라서:

> **ZCA ≠ optimistic init.** 표면("0이 음수를 흡수")은 같으나, optimistic init은 *"아직 안 배워서"*(일시적,
> 고정점=진짜값), ZCA는 *"구조적으로 못 배워서"*(영구적, 고정점=0≠진짜값). 이 *고정점의 차이*가 본 기여의 커널.
...
> **단서(메커니즘 정직성)**: VIC의 credit은 $\frac{\varepsilon}{\varepsilon+c}\mu_C$ 로 *부호·내용 의존*(진짜값 쪽으로 당김)인
> 반면, 무정보 처방(노이즈·tie-break)은 고정점 0을 바꾸지 않고 argmax만 흔든다.
...
1. **strict ZCA 발동 여부.** ZCA의 흡수는 $0 > Q(\text{BET})<0$ 인 *부등식*이지 *동률*이 아니다.
   ...
   **노이즈**도 고정점 0이 $Q(\text{BET})=-0.5$ 보다 *높아서*, 결정시 잡음을 줘도 열등 CHECK를 다수
...
2. **informed 여부.** VIC와 fixed-penalty는 둘 다 복원하나, VIC의 고정점은 $\frac{\varepsilon}{\varepsilon+c}\mu_C$ 로
   *진짜값 $\mu_C$ 부호·크기에 연동*(informed)인 반면 fixed-penalty는 상수 $-\kappa$ 로 *무정보*. 서로 다른
...
- 본 정리는 **존재·특성화**(ZCA 고정점이 생기고 열등 흡수를 만든다)이지, *"VIC가 유일·최선 처방"*이 아니다.
  VIC는 *임계 이상이면 작동하는 한 처방*이며(Prop. 7). 단, §7.1에서 **노이즈·tie-break은 strict ZCA를
...
- 따라서 방어 가능한 주장: **"투자비례 기여도 배분은 분산감소의 합리적 선택이나 비용 0 행동에 구조적
  영-고정점(ZCA)을 남기고(증명), 이는 optimistic init과 구별되며(Prop. 6), 최소 가상비용 VIC가 임계 이상에서
