# -*- coding: utf-8 -*-
"""3차 논문(v3) → MS Word(.docx). KCI [별표1] 양식.
서사: ZCA 양방향 오순위 진단 → 수동화 실측 → 임계 가상비용 실증(fixed-K) → 정직한 범위 규정.
실행: ../.venv/Scripts/python.exe build_docx_v3.py   (zca_theory/ 에서)
소스: zca_vic_논문초안_v3.md. 신명조 미설치 시 BODY_FONT='바탕'."""
...
para(doc,
     ...
     "(기여도 0 → credit 0)의 구조적 성질임을 보이며, 낙관적 초기화의 일시적 흡수와 질적으로 구별하고(영구·고정점≠"
     "참값), 해소에 필요한 임계 가상비용 ε_min을 유도한다. (실측) 2,048-셀 헤즈업 홀덤 봇에서 Q(CHECK)가 "
     "정확히 0으로 붕괴함을 관측하고(표준 MC는 −73~+120칩 분포), 그 정책적 결과가 수동화임을 행동 "
     "수준에서 보인다 — 턴 결정의 65%를 체크하는 소극 정책. (처방 실증) 1칩 가상비용은 회복을 재현하지 "
     "못하고 학습 seed에 따라 부호가 요동하지만(6-seed 평균 −117 mbb/g, 양수 2/6 — 전이구간), 임계를 "
     "넘는 상수 5칩 이상이면 수동화가 풀리고(턴 소액 베팅 65%로 전환) 이 병리를 착취하던 "
     "미학습 상대(무작위 정책)에 대한 성능이 학습 seed 전반에서 회복된다(−318±123 → +1546±535 mbb/g, "
     "0/6 → 6/6 양수). 결정시점 상수 비용 등 인과적으로 깨끗한 변형에서 재현되어 사후정보·구현 인공물 "
     "가설은 통제실험으로 배제하였다. 끝으로 이 효과가 일반화가 아님을 정직하게 보고한다 — 사전 고정한 "
     "비용으로 4종 미학습 상대에 검증한 결과 이득은 상대 의존적이며(콜링스테이션 상대는 소폭 손해), "
     "효과는 단일 상대 학습 구성에 한정된다.", size=9, space_after=4)
...
para(doc,
     "In tabular Monte-Carlo (MC) control, proportional credit assignment—sharing the terminal payoff "
     "in proportion to each action's invested chips—is a reasonable variance-reduction choice, but it "
     "leaves a structural pathology for zero-cost actions (e.g., CHECK): their credit is exactly zero "
     "every episode, pinning the MC fixed point at zero regardless of true value — Zero-Credit "
     "Absorption (ZCA). We treat this at three levels. (Theory) In a minimal toy MDP we prove the zero "
     "fixed point and the resulting bidirectional mis-ranking (overvaluing bad checks = absorption; "
     "undervaluing good checks = masking), show that the pathology is a structural property of the "
     "whole family of contribution-proportional schemes (zero contribution → zero credit), distinguish "
     "it from the transient absorption of optimistic "
     "initialization, and derive the threshold virtual cost ε_min required to dissolve it. "
     "(Measurement) In a 2,048-cell heads-up hold'em agent, the learned Q(CHECK) collapses exactly to "
     "zero (vs. −73 to +120 chips under standard MC), and the policy consequence is passivity — "
     "checking 65% of turn decisions. (Remedy) A one-chip cost fails to reproduce recovery, with its "
     "sign fluctuating across training seeds (mean −117 mbb/g, 2/6 positive — a transition zone), but a "
     "constant cost of five chips or more exceeds the threshold: passivity dissolves (65% small bets "
     "on the turn) and performance against the unseen opponent that exploited this pathology (a random "
     "policy) recovers across training seeds (−318±123 → +1546±535 mbb/g; 0/6 → 6/6 positive), "
     "reproducing under causally clean variants that rule out hindsight-information and "
     "implementation-artifact explanations. Finally, we honestly report that this is not "
     "generalization: with the cost fixed in advance, gains against four held-out opponents are "
     "opponent-dependent (a slight loss against a calling station), and the effect is confined to "
     "single-opponent training.", size=9, space_after=4)
...
para(doc, "Monte-Carlo 제어에서 종단 보상을 행동에 배분하는 방식은 학습 신호의 분산·편향을 좌우한다. "
          ...
          "비례 credit은 분자가 0이라 항상 0이고, 그 가치 추정의 고정점은 참값과 무관하게 0에 고정된다. 이 "
          "0은 행동 순위를 양방향으로 왜곡한다 — 참값이 음수인 체크는 과대평가되어 학습된 음수 행동들을 "
          "greedy에서 가리고(흡수), 참값이 양수인 체크는 과소평가되어 열등한 +EV 행동에 밀린다(은폐).", space_after=6)
para(doc, "본 논문의 기여는 다음과 같다. (1) 진단 — 위 현상을 영-기여 흡수(ZCA)로 명명해 toy MDP에서 "
          ...
          "2,048-셀 에이전트에서 Q(CHECK)의 정확한 영-고정점과 그 정책적 "
          "결과(턴 결정의 65%를 체크하는 수동화)를 측정한다(5장). (2) 임계 이론과 실증 — 비용 0 행동에 "
          "가상비용 ε을 부여하면 고정점이 참값 방향으로 풀리며, 해소에 필요한 임계 ε_min을 유도하고(4장), "
          "실제 시스템에서 1칩은 회복을 재현하지 못하고 부호가 seed에 따라 요동하며(전이구간)·임계를 "
          "충분히 넘는 상수 5칩 이상은 전 seed 유효함을 재현한다(5장) — 이론이 예측한 임계의 존재가 단조 "
          "용량-반응으로 확인된 사례다(임계의 위치까지 예측하는 "
          "것은 아니다 — 6장 한계 ①). 아울러 동일 예산의 표준 MC와 3자 비교로 처방의 실용적 가치 — "
          "저분산 배분을 보존한 채 병리만 제거 — 를 분리하고(5.3.1절), 표준 처방 후보(탐색 강화·낙관적 "
          "초기화·일률 벌점)가 전부 병리를 해소하지 못함을 통제 비교로 보인다(5.5절). (3) 정직한 범위 규정 — 이 회복이 "
          "\"일반화\"가 아니라 병리를 착취하던 상대에 대한 회복임을 사전 고정 비용의 홀드아웃 검증으로 "
          "보이고(콜링스테이션 상대는 소폭 손해), 평가에서 발견·정정한 구현 인공물과 단일 seed의 위험을 "
          "방법론적 부정적 결과로 보고한다(5·6장).", space_after=6)
...
para(doc, "기존 알고리즘의 병리를 명명·진단하는 연구는 강화학습에서 확립된 계보를 갖는다: Q-learning의 "
          ...
          "bias[17]·dormant neuron[18]처럼 병리 명명 → 진단 → 최소 처방의 형식이 반복된다. 본 연구는 같은 "
          "형식을 기여도 배분의 영-고정점에 적용한 것이다.", space_after=4)
...
para(doc, "Lemma 3 (계열 일반화). 임의의 기여도 함수 φ≥0에 대해 credit(a)=[φ(a)/Σφ]·P 꼴의 비례 credit은 "
          "φ(a)=0인 행동에서 Lemma 2와 동일한 영-고정점을 갖는다 — ZCA ⟺ φ(비용 0 행동)=0. 투자액"
          "(φ=inv)뿐 아니라 칩과 무관한 공격성 지표(φ=베팅 여부 1/0)에서도 재현되고, 균등 기여(φ≡1)와 "
          "표준 MC(return-equivalent)는 면역이다(수치 검증 스크립트 공개). 즉 ZCA는 본 연구가 채택한 특정 "
          "배분식의 결함이 아니라, null-player 공리(기여 0 → 배분 0)[3]를 가치 학습 신호로 쓰는 계열 "
          "전체의 구조적 함정이다.", space_after=4)
...
para(doc, "Proposition 2 (해소 임계). CHECK에 가상 투자 ε>0을 주면 Q_vic(CHECK)=[ε/(ε+c)]·μ_C로 고정점이 "
          ...
          "환경에서는 고정 소액 ε이 큰 팟에서 "
          "희석되므로(ε/(ε+c)→0), 임계는 대표 팟 규모 기준으로 설정해야 한다. 나아가 결정 상태(셀)마다 팟 "
          "규모가 달라 ε_min도 셀마다 다르므로, 고정 ε이 일부 셀에서만 임계를 넘는 중간 크기에서는 "
          "부분적·seed 의존적 회복(전이구간)이 예상된다 — 5장에서 1칩이 정확히 이 양상을 보인다.", space_after=6)
...
para(doc, "영-고정점의 행동적 결과를 보기 위해 학습된 정책의 행동 분포를 측정하면(20k 게임), 무개입 정책은 "
          "턴 결정의 65%를 체크한다. 공격 행동의 Q가 0 근방(약학습)이거나 음수인 셀에서 CHECK=0이 "
          "greedy를 차지하기 때문이다(Theorem의 실제 형태). 이 수동화는 그것을 착취하는 상대에게 "
          "체계적으로 진다 — 무작위 정책(미학습 상대) 대상 성능이 6개 학습 seed 전부에서 음수다(−318±123 "
          "mbb/g, 0/6 양수). 무작위 상대조차 이기지 못하는 것은 강함의 문제가 아니라 병리의 증거다(단순 "
          "공격 룰 정책은 같은 상대에 +11,705를 얻는다).", space_after=4)

h2(doc, "5.3 임계 실증 — 1칩은 전이구간, 5칩 이상은 유효")
para(doc, "가상비용의 크기를 단일변수로 스윕한 결과(<Table 2>), Proposition 2의 예측과 정합하게 용량-반응이 "
          "단조하며, 임계를 충분히 넘는 개입에서만 전 seed 양수가 된다.", space_after=4)
table(doc, [["개입 (CHECK 가상 invest)", "vs 무작위 (mbb/g)", "양수 seed", "판정"],
            ["(참조) 표준 MC — 배분 제거", "−312±91", "0/5", "병리 없음·양 지표 열세(5.3.1절)"],
...
para(doc, "¹ 1칩 조건의 초기 단일 seed(42) 측정은 −376으로 \"무효\"를 시사했으나, 6-seed 확장에서 부호 "
          "요동(2/6 양수)이 드러났다 — 단일 seed의 오도 위험(6장)의 추가 사례이며, 셀별 임계 이질성에 의한 "
          "전이구간 해석(4장)과 정합한다. 조건별 분포는 <Figure 1>과 "
...
para(doc, "4장의 이론적 기각(무정보 처방은 고정점을 옮기지 못함)을 동일 조건 통제 비교로 실증하였다(각 "
          ...
          "탐색 강화(softmax 온도 하한 2.0), ② 낙관적 초기화(전 셀 Q₀=+50 — Proposition 1의 검증), ③ "
          "일률 가산 벌점(전 행동의 credit에서 5칩 차감 — action-penalty[8]류의 비선택적 비용).", space_after=4)
table(doc, [["처방", "vs 무작위", "양수 seed", "vs TAG"],
      ...
      "<Table 4> 경쟁 처방 비교 / Rival remedies under identical protocol (mbb/g, 학습 seed 5–6개)")
para(doc, "² 5칩은 (i) ③ 일률 벌점과 크기를 일치시켜 선택성만을 분리하고, (ii) 임계를 넘는 최소 유효 "
          "용량이라는 보수적 대표값이다. 더 높은 수치의 조건(체크시점 팟 30% +1546±535·6/6, 상수 20칩 "
          "+1659±825·5/5)으로 바꿔도 결론은 불변하며, 조건 간 순위는 표준편차 중첩으로 주장하지 "
          "않는다(<Table 2>).", space_after=4)
...
para(doc, "(1) credit 폴백 인공물의 발견과 정정. 초기 구현은 총투자 0인 핸드(전부 체크/폴드)에서 비례 "
          ...
          "흘렸다. 단일 학습 seed와 결합해 \"1칩 가상비용이 분포 외 일반화의 필요조건\"이라는 허위 신호를 "
          "만들었으며, 폴백 격리런(원본 수치 완전 재현)과 제거 재실행(효과 소멸)으로 인공물임을 확정하고 "
          "폐기하였다 — 단일 seed 오도 위험[10][11][12]의 실제 사례. 본 논문의 모든 수치는 폴백 제거 후의 "
          "것이며, 나아가 개입을 실투자 핸드에만 적용하는 격리 실험으로 5장의 회복이 폴백 유사 신호가 "
          "아니라 CHECK credit 경로임을 확인했다(실투자-한정 +1245(5/5) ≈ 전체 적용 +1546; "
          "폴백-신호-한정 +206·고분산).", space_after=4)
para(doc, "(2) 한계. ① 임계의 정확한 위치는 미확정 — 전이구간(팟 8~15%·상수 1칩 부근)은 seed 요동이 "
          "크며(1칩 6-seed 평균 −117·2/6 양수), \"충분 초과 시 전 seed 유효\"라는 임계의 존재만 주장한다. ② 효과의 범위는 단일 상대 학습·단일 "
          "게임(HUNL)·단일 추상화에 한정된다. ③ 홀드아웃 상대도 자작 룰 정책이며 외부·균형 상대(예: 근사 "
          "GTO)에 대한 검증은 향후 과제다. ④ ZCA의 부품(null-player 공리·return-equivalence·낙관적 "
          "초기화·PBRS≡Q-init)은 기지이며, 기여는 이들을 실패 모드로 재진단하고 임계를 유도·실증하며 "
          "범위를 정직히 규정한 데 있다.", space_after=6)

h1(doc, "7. 결론 및 향후 연구")
para(doc, "비례배분 기여도는 비용 0 행동에 구조적 영-고정점(ZCA)을 남기고, 이는 행동 순위를 양방향으로 "
          "왜곡하며(흡수·은폐), 낙관적 초기화의 일시적 0-선호와 달리 방문으로 해소되지 않는다 — 이상을 "
          "toy MDP에서 증명하고 실제 2,048-셀 에이전트에서 Q(CHECK)=0의 정확한 붕괴와 그 결과인 정책 수동화(턴 "
          "체크 65%)로 실측하였다. 유도한 임계 조건의 예측은 단조 용량-반응으로 확인되었다: 무개입은 전 "
          "seed 음수(0/6), 1칩은 부호가 요동하는 전이구간(2/6), "
          "상수 5칩 이상이면 수동화가 풀리고(턴 소액 베팅 65%) 병리를 착취하던 미학습 상대에 대한 성능이 "
          "학습 seed 전반에서 회복된다(5/5·6/6 양수). 확인된 것은 임계의 존재이며 위치의 예측이 "
          "아니다(6장 한계 ①). 사후정보·구현 인공물·평가 순환성 가설은 각각 "
          "결정시점 상수 비용, 격리 실험, 사전 고정 홀드아웃으로 배제하였고, \"표준 MC를 쓰면 된다\"는 "
          "대안은 동일 예산 3자 비교(양 지표 열세)로 기각하였다. 동시에 이 회복이 일반화가 "
          "아니라 상대 의존적 스타일 변화임을 — 콜링스테이션 상대의 소폭 손해까지 — 정직하게 보고하였다. "
          "본 사례연구의 가치는, 구조적 기여도 배분의 병리를 증명·측정하고, 처방의 작동 조건(임계)을 "
          "이론-실험 대응으로 규명하며, 그 한계를 부정적 결과와 함께 드러낸 데 있다.", space_after=6)
para(doc, "향후 연구. ZCA의 발생 조건은 게임 특수적이지 않다 — ① 성과가 말단에 한 번 실현되고, ② 행동이 "
          ...
          "효용)에서 성립한다. 두 환경에서의 검증 — 특히 요금 구조가 균질한 V2G에서의 임계 위치 사전 계산, "
          "sleep의 대기전력이라는 실재 비용에 의한 가상비용의 물리적 해석 — 을 후속 연구로 진행한다.", space_after=8)
