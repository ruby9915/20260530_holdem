...
para(doc,
     ...
     "체크의 과소평가 = 은폐)를 증명하고, 낙관적 초기화의 일시적 흡수와 질적으로 구별하며(영구·고정점≠"
     "참값), 해소에 필요한 임계 가상비용 ε_min을 유도한다. (실측) 2,048-셀 헤즈업 홀덤 봇에서 Q(CHECK)가 "
     "정확히 0으로 붕괴함을 관측하고(표준 MC는 −73~+120칩 분포), 그 정책적 결과가 수동화임을 행동 "
     "수준에서 보인다 — 턴 결정의 65%를 체크하는 소극 정책. (처방 실증) 임계 미달인 1칩 가상비용은 "
     "무효지만, 임계를 넘는 상수 5칩 이상이면 수동화가 풀리고(턴 소액 베팅 65%로 전환) 이 병리를 착취하던 "
...
para(doc,
     ...
     "undervaluing good checks = masking), distinguish it from the transient absorption of optimistic "
     ...
     "checking 65% of turn decisions. (Remedy) A sub-threshold one-chip cost is ineffective, but a "
...
para(doc, "표 기반 강화학습은 함수근사의 블랙박스 효과 없이 학습 동역학을 단일변수로 통제·관찰할 수 있다. "
          ...
          "봇이 아니라 구조적 기여도 배분이 남기는 측정 가능한 병리의 진단과, 그 병리를 해소하는 최소 "
          "개입의 임계 조건 규명이다.", space_after=6)
...
para(doc, "본 논문의 기여는 다음과 같다. (1) 진단 — 위 현상을 영-기여 흡수(ZCA)로 명명해 toy MDP에서 "
          "양방향 오순위로 증명하고(3장), 실제 2,048-셀 봇에서 Q(CHECK)의 정확한 영-고정점과 그 정책적 "
          "결과(턴 결정의 65%를 체크하는 수동화)를 측정한다(5장). (2) 임계 이론과 실증 — 비용 0 행동에 "
          "가상비용 ε을 부여하면 고정점이 참값 방향으로 풀리며, 해소에 필요한 임계 ε_min을 유도하고(4장), "
          "실제 시스템에서 임계 미달(1칩)은 무효·임계 초과(상수 5칩 이상)는 유효함을 학습 seed 전반에서 "
          "재현한다(5장) — 이론의 예측이 실험으로 확인된 사례다. (3) 정직한 범위 규정 — 이 회복이 "
...
para(doc, "Proposition 2 (해소 임계). CHECK에 가상 투자 ε>0을 주면 Q_vic(CHECK)=[ε/(ε+c)]·μ_C로 고정점이 "
          ...
          "검증 일치). 임계 미달이면 Q_vic(CHECK)≈0으로 오순위가 잔존한다 — 이 예측이 5장에서 \"1칩 무효, "
          "5칩 이상 유효\"로 확인된다. 팟 규모 c가 궤적마다 변하는 환경에서는 고정 소액 ε이 큰 팟에서 "
          "희석되므로(ε/(ε+c)→0), 임계는 대표 팟 규모 기준으로 설정해야 한다.", space_after=6)
...
h2(doc, "5.3 임계 실증 — 1칩은 무효, 5칩 이상은 유효")
para(doc, "가상비용의 크기를 단일변수로 스윕한 결과(<Table 2>), Proposition 2의 예측대로 임계 미달 개입은 "
          "무효, 임계 초과 개입은 유효하다.", space_after=4)
table(doc, [["개입 (CHECK 가상 invest)", "vs 무작위 (mbb/g)", "양수 seed", "판정"],
            ["없음 (ε=0)", "−318±123", "0/6", "수동화(기준선)"],
            ["상수 1칩", "−376", "0/1¹", "임계 미달 — 무효"],
...
para(doc, "(2) 한계. ① 임계의 정확한 위치는 미확정 — 전이구간(팟 8~15%·1~5칩 사이)은 seed 요동이 커 "
          "\"임계 미달 무효/충분 초과 유효\"의 존재만 주장한다. ② 효과의 범위는 단일 상대 학습·단일 "
...
para(doc, "비례배분 기여도는 비용 0 행동에 구조적 영-고정점(ZCA)을 남기고, 이는 행동 순위를 양방향으로 "
          ...
          "toy MDP에서 증명하고 실제 2,048-셀 봇에서 Q(CHECK)=0의 정확한 붕괴와 그 결과인 정책 수동화(턴 "
          "체크 65%)로 실측하였다. 유도한 임계 가상비용 조건은 실험으로 확인되었다: 임계 미달 1칩은 무효, "
          "상수 5칩 이상이면 수동화가 풀리고(턴 소액 베팅 65%) 병리를 착취하던 미학습 상대에 대한 성능이 "
          "학습 seed 전반에서 회복된다(0/6 → 6/6 양수). 사후정보·구현 인공물·평가 순환성 가설은 각각 "
          "결정시점 상수 비용, 격리 실험, 사전 고정 홀드아웃으로 배제하였다. 동시에 이 회복이 일반화가 "
