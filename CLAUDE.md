...
> **표기 단일 출처 = 루트 [용어집.md](용어집.md) (2026-07-08 신설).** 이 절과 용어집이 충돌하면 **용어집이 우선**한다. 핵심 두 원칙: ① 포커 도메인 용어는 음차 유지·직역 금지(핸드·팟·스택·블러프·에퀴티), ② 학술 용어는 KCI 번역어(잡음·기준선·수렴), ③ 알고리즘명·약어는 원어. street는 본 연구에서 **"라운드"로 통일**. 표에 없는 용어는 용어집 §0 원칙으로 판단 후 용어집에 추가.
...
- **VIC**: 비용 0 행동(CHECK)에 가상 투자를 부여해 영-고정점 흡수를 해소 — 상수 K칩(`fixed`) 또는 α×팟(`checktime`/`terminal`), **임계(ε_min) 초과 시 유효** (초기 1칩 설계는 임계 미달로 전이구간 — 실험일지 33절 E7)
