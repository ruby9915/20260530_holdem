...
- **ZCA (Zero-Credit Absorption)**: 비례배분(PROP) 기여도에서 비용 0인 CHECK는 credit≈0 → Q(CHECK)가 ±1.4칩 면도날 띠로 붕괴(0-고정점). 이 0이 음수 행동을 greedy로 흡수.
...
- **OOD/ID 해리**: VIC를 끄면 vs Random(OOD)만 선택적으로 붕괴(−346~−3,339)하고 vs TAG(ID)는 +로 유지(부호반전). 이것이 핵심 규명 기여.
...
3. **결론은 100k×5 정밀 평가에서만 신뢰.** 200게임 체크포인트 평가는 vs Random에서 노이즈 지배 → 부호반전 흔함. vs Random 주장은 100k×5에서만.
...
> **표기 단일 출처 = 루트 [용어집.md](용어집.md) (2026-07-08 신설).** 이 절과 용어집이 충돌하면 **용어집이 우선**한다. 핵심 두 원칙: ① 포커 도메인 용어는 음차 유지·직역 금지(핸드·팟·스택·블러프·에퀴티), ② 학술 용어는 KCI 번역어(노이즈·기준선·수렴), ③ 알고리즘명·약어는 원어. street는 본 연구에서 **"라운드"로 통일**. 표에 없는 용어는 용어집 §0 원칙으로 판단 후 용어집에 추가.
...
| 셋업(setup) | 설정, 구성 |
| 케이스(case) | 경우, 사례 |
| ~~노이즈(noise)~~ | ~~잡음~~ → **노이즈** (저자 선호 확정 2026-08-20, 용어집 우선) |
| 로버스트(robust) | 강건한 / 강건성 |
| 컨버전스 | 수렴 |
| 트레이드오프 | 절충, 상충 관계 |
| 디스소시에이션 | 해리 |
...
**유지 확정**(현행이 정확 — 바꾸지 말 것): 몬테카를로 **제어**(control problem; '통제' 아님), **통제실험/통제비교**(단일변수 통제; '대조'는 대조군 오해), **결정 상태**(MDP 상태; '결정 노드' 아님), 고정점·흡수·일반화·사례연구·재현성·수렴 (진단은 2026-08-24 저자 메모로 '규명'으로 교체 확정 — 용어집 §6). **greedy**는 원어 유지(용어집 합의; '탐욕적 정책' 아님).
...
- **VIC**: 비용 0 행동(CHECK)에 가상 투자를 부여해 0-고정점 흡수를 해소 — 상수 K칩(`fixed`) 또는 α×팟(`checktime`/`terminal`), **임계(ε_min) 초과 시 유효** (초기 1칩 설계는 임계 미달로 전이구간 — 실험일지 33절 E7)
- **ZCA**: Zero-Credit Absorption — 비용 0 행동의 0-credit이 argmax를 흡수하는 문제
