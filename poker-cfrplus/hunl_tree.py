...
SB, BB, STACK = 1, 2, 200
# 2026-07-15 확정 설계 C: 매 라운드 첫 벳(=1레벨)만 학습자 A12 8종 전체,
# 리레이즈 이상은 {팟}+올인. 경위: advance()의 level=0 버그로 포스트플랍 첫 벳이
# 팟 1종으로 붕괴돼 있었음(번역 16~27%의 진범) → 수정하니 전 지점 8종은 정보집합
# 1.6억(불가) → 첫 벳 8종/리레이즈 팟만(정보집합 1,200만)으로 확정. 번역은 드문
# 리레이즈에만 잔존(계수 공개). 실험일지 35절 보강 기록.
MENU = {1: (0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0)}
MENU_DEEP = (1.0,)                      # 2레벨(리레이즈) 이상
