...
SB, BB, STACK = 1, 2, 200
# 2026-07-15 설계 A (저자 지시 — C안 LAG 적자 −250 대응 해상도 업):
# 첫 벳 = 학습자 A12 8종, 리레이즈 = {0.5, 1}×팟, 3-벳 이상 = {팟}+올인.
# 결정 731k·정보집합 3,660만·솔빙 ~33h. 경위(레벨 버그·C안 게이트 FAIL)는
# git 이력과 실험일지 35절 보강 참조.
# 2026-07-18 설계 B3 (저자 승인 ⓐ): 리레이즈 = {0.25, 0.5, 0.75, 1} — LAG-스냅
# 실험이 지목한 착취 크기(0.25·0.75) 포함. 학습자 리레이즈 주력 크기 정합.
MENU = {1: (0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 4.0),
        2: (0.25, 0.5, 0.75, 1.0)}
MENU_DEEP = (1.0,)                      # 3레벨(3-벳) 이상
...
def build():
    ...
    def make_decision(street, to_act, contrib, owed, last_raise, level,
        ...
        def advance(new_contrib):
            ...
            stats['chance'] += 1
            nxt = make_decision(street + 1, 1, new_contrib, 0, BB, 1, 0, '')
