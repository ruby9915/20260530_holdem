"""
train_eval.py (MC + 즉각 비용 버전)
─────────────────────────────────────────────────────────────────────
학습 진행에 따른 성능 변화 측정.

  [TRAIN N episodes (MC)] → [EVAL vs Random 200게임] → [EVAL vs RuleBased 200게임]

평가 시 ε=0 (순수 greedy), Q-테이블 업데이트 없음.
결과는 콘솔 + CSV 저장.
"""
import csv
import math
...
# (라운드, facing_bet) → 강도별 행동. 핸드 강도 8단계.
_RULE_POLICY = {
    (Round.PREFLOP, False): {  # 오픈: 작은 팯 → 큰 비율
