"""
train_eval.py (MC + 利됯컖 鍮꾩슜 踰꾩쟾)
?????????????????????????????????????????????????????????????????????
?숈뒿 吏꾪뻾???곕Ⅸ ?깅뒫 蹂??痢≪젙.

  [TRAIN N episodes (MC)] ??[EVAL vs Random 200寃뚯엫] ??[EVAL vs RuleBased 200寃뚯엫]

?됯? ??琯=0 (?쒖닔 greedy), Q-?뚯씠釉??낅뜲?댄듃 ?놁쓬.
寃곌낵??肄섏넄 + CSV ???
"""
import csv
import math
...
# (?쇱슫?? facing_bet) ??媛뺣룄蹂??됰룞. ?몃뱶 媛뺣룄 8?④퀎.
_RULE_POLICY = {
    (Round.PREFLOP, False): {  # ?ㅽ뵂: ?묒? ??????鍮꾩쑉
