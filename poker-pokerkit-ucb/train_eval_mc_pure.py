"""
train_eval_mc_pure.py  (Pure MC ??利됯컖 鍮꾩슜 ?쒓굅)
?????????????????????????????????????????????????????????????????????
?먯깋 ?뺤콉: UCB1 (湲곗〈 train_eval_mc.py ? ?숈씪)

MC ?낅뜲?댄듃: ?쒖닔 MC (immediate cost 誘몄궗??
    payoff = 理쒖쥌 ?ㅽ깮 - ?쒖옉 ?ㅽ깮   (?밸━ ??+?묒닔, ?⑤같 ??-?묒닔, ?移?
    G = payoff
    for (r, s, a) in reversed(trace):
        Q(s, a) ??Q(s, a) + 慣 [ G - Q(s, a) ]
        G = 款 * G    # ?댁쟾 ?ㅽ뀦?쇰줈 媛硫??좎씤留??곸슜

?듭떖: imm ?뚯닔 ?⑤꼸?곌? ?щ씪吏怨?payoff ?먯껜媛 ?移?씠??
...
# ?????????????????????????????????????????????????????
# ?먰뵾?뚮뱶 ?ㅽ뻾 (?숈뒿????Pure MC, imm 誘몄궗??
# ?????????????????????????????????????????????????????
def play_train_episode(ql: QLearning, learner_id: int = 0) -> float:
    """
    UCB濡??됰룞 ?좏깮, ?먰뵾?뚮뱶 醫낅즺 ??payoff ?섎굹留???갑???꾪뙆.
    媛??≪뀡 G = 款^(d) * payoff, d = 洹??≪뀡 ?댄썑 ?⑥? ?≪뀡 ??
    """
    pk_state = _make_game()
