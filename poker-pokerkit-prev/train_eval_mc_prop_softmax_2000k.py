"""
train_eval_mc_prop_softmax_2000k.py  (비례 배분 MC + Softmax 탐색 + PrevAction + CHECK=1chip)
─────────────────────────────────────────────────────────────────────
16번 실험(train_eval_mc_prop_check1_1200k.py)을 기반으로 하되,
탐색 정책을 ε-greedy 대신 Softmax(볼츠만) 탐색으로 전면 교체하여
2,000,000 에피소드 동안 학습 및 정밀 평가를 수행한다.

  ▸ CHECK_VIRTUAL_INVEST = 1  (1 chip 가상 정보비용)
  ▸ 탐색 함수: ql.softmax_action
  ▸ 온도 스케줄: TEMP_START = 10.0 -> TEMP_END = 0.5 (80% 구간 선형 감소)
  ▸ 에피소드: 2,000,000 (200만)
  ▸ 평가 주기: 10,000 에피소드마다 (200 체크포인트)
"""
import csv
import math
import random
...
TEMP_START      = 10.0
TEMP_END        = 0.5
TEMP_DECAY_END  = 0.8    # 전체의 80% 구간에서 선형 감소
