"""
train_eval_mc_prop_softmax_2000k.py  (鍮꾨? 諛곕텇 MC + Softmax ?먯깋 + PrevAction + CHECK=1chip)
?????????????????????????????????????????????????????????????????????
16踰??ㅽ뿕(train_eval_mc_prop_check1_1200k.py)??湲곕컲?쇰줈 ?섎릺,
?먯깋 ?뺤콉??琯-greedy ???Softmax(蹂쇱툩留? ?먯깋?쇰줈 ?꾨㈃ 援먯껜?섏뿬
2,000,000 ?먰뵾?뚮뱶 ?숈븞 ?숈뒿 諛??뺣? ?됯?瑜??섑뻾?쒕떎.

  ??CHECK_VIRTUAL_INVEST = 1  (1 chip 媛???뺣낫鍮꾩슜)
  ???먯깋 ?⑥닔: ql.softmax_action
  ???⑤룄 ?ㅼ?以? TEMP_START = 10.0 -> TEMP_END = 0.5 (80% 援ш컙 ?좏삎 媛먯냼)
  ???먰뵾?뚮뱶: 2,000,000 (200留?
  ???됯? 二쇨린: 10,000 ?먰뵾?뚮뱶留덈떎 (200 泥댄겕?ъ씤??
"""
import csv
import math
import random
...
TEMP_START      = 10.0
TEMP_END        = 0.5
TEMP_DECAY_END  = 0.8    # ?꾩껜??80% 援ш컙?먯꽌 ?좏삎 媛먯냼

# ?? ?됯? ?ㅼ젙 ?????????????????????????????????????????
EVAL_EVERY      = 10_000
EVAL_GAMES      = 200
CSV_PATH        = "eval_results_mc_prop_softmax_2000k.csv"
...
def temperature_at(episode: int) -> float:
    progress = min(1.0, episode / (TOTAL_EPISODES * TEMP_DECAY_END))
...
def main():
    ...
    sep = "-" * len(hdr)
    print(sep)
    print(f"  CHECK=1chip virtual invest  |  Prop MC Softmax + PrevAction  |  2,000,000 ep")
    print(sep)
    print(hdr)
    print(sep)
    ...
    print(f"{0:>9} {'0.0%':>5} {TEMP_START:>5.1f} |"
    ...
    while ep < TOTAL_EPISODES:
        ...
        cur_temp = temperature_at(ep)
        print(f"{ep:>9} {pct:>4.1f}% {cur_temp:>5.2f} |"
              f" {wr*100:>5.1f}% {mr:>+9.0f} |"
              f" {wrb*100:>5.1f}% {mrb:>+9.0f} |"
    ...
    print("\n=== Q-Table (Prop MC + Softmax + CHECK=1chip + PrevAction) ===")
    ql.print_q_table()
