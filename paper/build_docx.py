# -*- coding: utf-8 -*-
"""zca_vic 논문 초안을 MS Word(.docx)로 생성. python-docx 사용.
실행: ../.venv/Scripts/python.exe build_docx.py  (paper/ 에서)
수식은 유니코드(μ ε ≈ ≠ → ∀ ∈ − ₁ ₂)로 표기."""
...
KFONT = "Malgun Gothic"   # 한글 폰트


def kfont(run, size=10, bold=False, italic=False, color=None):
...
def para(doc, text="", size=10, bold=False, italic=False, align=None, space_after=6):
    ...
    if text:
        kfont(p.add_run(text), size, bold, italic)
    return p


def heading(doc, text, size=12, space_before=10):
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(space_before)
    p.paragraph_format.space_after = Pt(4)
    kfont(p.add_run(text), size, bold=True)
    return p
...
def table(doc, rows, header=True, widths=None):
    ...
    for i, row in enumerate(rows):
        for j, cell in enumerate(row):
            ...
            kfont(c.paragraphs[0].add_run(str(cell)), 9, bold=(header and i == 0))
    return t
...
# ── 제목 ──
para(doc, "비례배분 기여도 하의 영-기여 흡수(Zero-Credit Absorption) 진단과 최소 가상비용 처방",
     size=15, bold=True, align=C, space_after=2)
para(doc, "— tabular Monte-Carlo 포커 학습의 형식적 사례연구 —", size=12, bold=True, align=C, space_after=6)
para(doc, "Diagnosing Zero-Credit Absorption under Proportional Credit Assignment and a Minimal "
          "Virtual-Cost Remedy: A Formal Case Study in Tabular Monte-Carlo Poker Learning",
     size=10.5, italic=True, align=C, space_after=8)
para(doc, "[저자명]¹", size=10, align=C, space_after=0)
para(doc, "¹[소속]", size=9, align=C, space_after=10)

# ── 국문 요약 ──
heading(doc, "국문 요약", 11)
para(doc,
     ...
     "credit은 매 에피소드 정확히 0이므로 그 Monte-Carlo 고정점은 행동의 진짜 가치와 무관하게 0에 "
     "고정되고, 모든 가용 행동이 음(−)의 기대값인 노드에서 이 0은 열등한 비용 0 행동을 greedy 정책이 "
     "영구히 선택하게 만든다. 우리는 이를 영-기여 흡수(Zero-Credit Absorption, ZCA)로 명명하고 최소 "
     "toy MDP에서 (i) 영-고정점, (ii) 흡수 정리, (iii) optimistic initialization의 일시적 흡수와의 질적 "
     "구별(영구적·고정점≠진짜값)을 증명한다. 또한 무시 가능한 가상비용(Virtual Information Cost, VIC)이 "
     "흡수를 해소함을 보이고 그 임계 가상비용을 유도하며, 통제비교로 무정보 노이즈·동점 처리(tie-break)가 "
     "흡수를 탈출하지 못함을 보인다. 본 결과는 학습 seed·평가 표본과 무관한 고정점 사실이며, 실제 다단계 "
     "MDP의 성능 효과 및 분포 외(OOD) 일반화 인과는 검증되지 않은 향후 과제로 정직하게 분리한다.")
p = doc.add_paragraph(); p.paragraph_format.space_after = Pt(10)
kfont(p.add_run("주제어: "), 9.5, bold=True)
kfont(p.add_run("강화학습, Monte-Carlo 제어, 기여도 배분, 보상 정형화, 영-기여 흡수, tabular Q-learning, "
                "헤즈업 노리밋 홀덤"), 9.5)

# ── Abstract ──
heading(doc, "Abstract", 11)
para(doc,
     ...
     "sample; the full-MDP performance effect and any link to out-of-distribution (OOD) generalization "
     "are honestly separated as future work.")
p = doc.add_paragraph(); p.paragraph_format.space_after = Pt(10)
kfont(p.add_run("Keywords: "), 9.5, bold=True)
kfont(p.add_run("reinforcement learning, Monte-Carlo control, credit assignment, reward shaping, "
                "zero-credit absorption, tabular Q-learning, heads-up no-limit hold'em"), 9.5)
...
para(doc, "처방 측면에서 VIC는 potential-based reward shaping(PBRS)이 Q-value 초기화와 등가[6][7]인 틀 "
          ...
          "오도 위험은 재현성 문헌[10][11][12]이 확립했으며, 본 연구는 이를 V장에서 음성 결과로 보고한다.")

# ── III. 형식적 특성화 ──
heading(doc, "III. 영-기여 흡수의 형식적 특성화")
para(doc, "문제 설정(toy MDP). 결정 상태 s에서 행동 a₁을 한 번 선택한다. 각 행동은 투자액 inv(a₁)≥0을 "
          ...
          "포함하고 종단 보상 P로 끝난다(γ=1, 종단 보상만). μ(a₁):=E[P|a₁]로 두면 진짜값은 q*(s,a₁)=μ(a₁)"
          "이다. 세 배분 방식의 행동별 return R(s,a₁)은 표 1과 같고, MC 업데이트 Q←Q+α(R−Q)의 고정점은 "
          "E[R]이다(sample-average 또는 감소 α; 상수 α도 평균 보존).")
para(doc, "표 1. 행동별 return R(s, a₁)", size=9, bold=True, space_after=2)
table(doc, [["방식", "R(s, a₁)"],
            ["표준 MC", "P"],
...
para(doc, "Lemma 2 (비용 0 행동의 영-고정점). Q_prop(s,a₁) = [inv(a₁)/(inv(a₁)+c)]·μ(a₁), 특히 "
          ...
          "고정점은 진짜값과 무관하게 0이며, R≡0이라 표본 분산도 0인 구조적 고정점(초기화 잔재 아님)이다. "
...
para(doc, "흡수의 적용 범위(중요). 흡수는 덜 나쁜 대안조차 −EV일 때만 발생한다. 어떤 행동이 +EV이면 "
          ...
          "노드에서 덜 나쁜 것을 고르는 능력\"뿐이다.", italic=False)
para(doc, "Proposition 1 (optimistic initialization과의 구분). Q를 0으로 초기화한 표준 MC에서 미방문 "
          "행동의 0-선호는 충분한 방문 후 Q→μ로 소거되어 일시적이며 고정점은 진짜값이다(Lemma 1). "
          "대조적으로 Q_prop(CHECK)=0은 수렴 후에도 유지되는 고정점이며 진짜값과 다르다(Lemma 2). 즉 "
          "표면(\"0이 음수를 흡수\")은 같으나 optimistic init은 \"아직 학습 못 해서\"(일시적, 고정점=진짜값), "
          "ZCA는 \"구조적으로 학습 불가라서\"(영구, 고정점=0≠진짜값)이다. 이 고정점의 차이가 본 진단의 "
          "핵심이다.")

# ── IV. VIC ──
heading(doc, "IV. VIC: 임계 가상비용과 탈출 메커니즘 비교")
para(doc, "Proposition 2 (VIC 복원과 임계). CHECK에 가상 투자 ε>0을 주면 Q_vic(CHECK)=[ε/(ε+c)]·μ_C이다. "
          ...
          "작아 \"무시할 1칩이 임계만 넘으면 충분\"하다는 설계와 정합한다. 핵심은 VIC가 부여하는 값이 진짜값 "
          "μ_C에 연동(informed)된다는 점이다.")
para(doc, "탈출 메커니즘 통제비교(표 2). 비례배분 위에 VIC, 무정보 노이즈(결정 시 N(0,σ²) 가산), "
          "tie-break(정확히 동률일 때만 CHECK 후순위), 고정 페널티(−κ)를 얹어 비교한다.")
para(doc, "표 2. 탈출 메커니즘 비교  (μ_C=−5, μ_B=−1, b=c=1)", size=9, bold=True, space_after=2)
...
# ── V. 검증·한계 ──
heading(doc, "V. 수치 검증과 한계")
para(doc, "검증. 표 1·2의 모든 주장을 독립 Monte-Carlo 시뮬레이션(40만 에피소드)으로 검증하였고 결과는 "
          "표 3 및 해석식과 일치한다. 특히 비례배분에서 Q(CHECK)=0.000 (≠ μ_C=−5)으로 흡수가, VIC에서 "
          "−2.50으로 복원이 재현된다. 임계도 ε_min=0.111에서 ε≤0.111→CHECK·ε>0.111→BET으로 소수점까지 "
          "일치한다. 검증 코드는 공개한다.")
para(doc, "표 3. 수치 검증 (40만 에피소드, b=c=1, μ_C=−5, μ_B=−1, ε=1)", size=9, bold=True, space_after=2)
...
para(doc, "범위와 한계(정직). (1) 본 결과는 학습 seed·평가 표본과 무관한 고정점 사실이다. 실제 2,048-셀 "
          ...
          "정합하는 음성 결과로 보고한다. 비용 0의 영-고정점(seed 무관)과 OOD 성능(seed 의존)은 별개 "
          "층위다. (3) VIC는 임계 이상이면 작동하는 한 처방이며 유일·최선이 아니다(IV장의 고정 페널티 대비 "
          "성능 우열은 미검증). (4) ZCA의 부품은 알려져 있고 VIC는 PBRS≡Q-init[7] 틀 안에 있어 알고리즘 "
          "신규성은 제한적이다 — 기여는 알려진 구조의 미보고 실패 모드를 명명·형식화하고 임계를 유도하며 "
          "범위를 정직히 분리한 데 있다. (5) 단일 추상화·단일 게임에 한정된다.")

# ── VI. 결론 ──
heading(doc, "VI. 결론")
para(doc, "본 연구는 tabular Monte-Carlo 비례배분 기여도가 비용 0 행동에 남기는 구조적 병리를 영-기여 "
          "흡수(ZCA)로 명명하고, toy MDP에서 영-고정점과 흡수를 증명하였다. ZCA는 optimistic "
          "initialization과 고정점 수준에서 질적으로 구별되며, 최소 가상비용 VIC가 명시적 임계 이상에서 "
          ...
          "가능한 병리를 형식적으로 진단하고 한계를 정직하게 드러낸 데 있다.")
...
refs = [
    ...
    '[5] J. A. Arjona-Medina et al., "RUDDER: Return decomposition for delayed rewards," in Advances in Neural Information Processing Systems, 2019, pp. 13544–13555.',
    '[6] A. Y. Ng, D. Harada, and S. Russell, "Policy invariance under reward transformations: Theory and application to reward shaping," in Proc. ICML, 1999, pp. 278–287.',
    ...
    '[9] T. Rashid, B. Peng, W. Boehmer, and S. Whiteson, "Optimistic exploration even with a pessimistic initialisation," in Proc. ICLR, 2020.',
    '[10] P. Henderson et al., "Deep reinforcement learning that matters," in Proc. AAAI Conf. Artificial Intelligence, 2018, pp. 3207–3214.',
    ...
    '[12] R. Agarwal, M. Schwarzer, P. S. Castro, A. C. Courville, and M. G. Bellemare, "Deep reinforcement learning at the edge of the statistical precipice," in NeurIPS, 2021, pp. 29304–29320.',
...
# ── 저자소개 ──
heading(doc, "저자소개")
para(doc, "[저자명] — [소속/직위]. 관심 분야: 강화학습, 게임 AI, 학습 동역학 분석.", size=9)

out = "zca_vic_논문초안.docx"
doc.save(out)
print("saved:", out)
