...
| 21a | [`21a_softmax_random_2000k`](21a_softmax_random_2000k/) | MC Prop Softmax + PrevAction + CHECK=1chip | Softmax | payoff × invest 비율 | **2000k** | (vs 19) 상대→**Random 단독** | vsRand mbb +737, vsRule mbb **-915** — Random 단독 학습은 RuleBased 일반화 안 됨 |
...
- **21a/21b/21c번** — 상대 유형(Random/RuleBased/Self-play)을 단독으로 고정했을 때의 학습 효과를 비교하는 3-way ablation. **21b(RuleBased 단독)만 vsRule +762 흑자**이고, 나머지 둘은 모두 vsRule 음수. 이로써 19번 흑자가 혼합 상대 덕분이 아니라 **RuleBased 상대에 특화 과적합**한 결과임을 입증. 깊은 셀(턴·리버) 도달률 분석(reachability 보정)에서 구조적 불가능 셀 45.5% 제거 후 실제 방문 가능 셀의 도달률은 78.8~91.2% → 커버리지 병목이 예산 부족이 아니라 **상대 분포 쏠림**임을 정량 확인.
