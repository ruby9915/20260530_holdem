...
> vsTAG(ID)는 전 조건 +869~923로 평평(보존). **checktime(결정시점 팟, 미래정보 없음)이 α=30%에서 terminal과 동급 재현** → hindsight 비판 소거. α=10%에서 checktime이 약한 것은 체크시점 팟<최종 팟이라 실효 ε이 작기 때문(임계 해석과 정합).
> **미해결 (야간 체인 E1~E6 진행 중, 실험일지 32절):** ① 올체크-핸드 균등분배 신호(옛 누수와 동일)가 pot-VIC에 재도입돼 있어 격리 필요 ② α가 vs Random으로 튜닝됨(순환) → 홀드아웃 페르소나 검증 ③ "일반화"가 아니라 "약체 착취 회복"일 가능성(Maniac 봇 +11,705 대비 13%) ④ 집계 흡수율은 α 무반응 — 메커니즘 미규명.
