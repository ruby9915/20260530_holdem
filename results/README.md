...
## VIC ablation (= 28번 실험, 논문 핵심)

[`ablation_vic_2m/`](ablation_vic_2m/) — **가상 정보 비용(VIC, CHECK에 1칩 가상 투자)** 의 인과 효과를 격리하는 2×3 ablation. 학습 방식(single / cycle / mixed) × VIC(on/off)를 각각 2M 에피소드 학습 후 **100k 게임 × 5회** 평가. 요약은 [`ablation_vic_2m/_summary_100kx5.csv`](ablation_vic_2m/_summary_100kx5.csv).
...
> **핵심**: VIC를 끄면 세 방식 모두 vsRandom이 큰 적자로 붕괴(−346 ~ −3,339)하지만 vsTAG는 +791~+815로 유지된다. CHECK 흡수 고정점이 **미학습 상대(Random)에 대한 일반화만 선택적으로 파괴**함을 인과적으로 입증.

## Pure-vs-Prop 통제실험 (= 29번)

[`pure_softmax_mixed_2m/`](pure_softmax_mixed_2m/) — 28번 mixed와 **100% 매칭, 기여도 배분만 PROP→PURE(표준 MC, 말단보상 γ-할인 역전파)** 단일 변경. 100k×5(eval seed 1000–1004). 요약 [`_pure_vs_prop_100kx5.log`](_pure_vs_prop_100kx5.log).
...
[`softmax_novic_random_2m/`](softmax_novic_random_2m/) — 19번(softmax+VIC **on**)과 **100% 매칭, `CHECK_VIRTUAL_INVEST`만 1→0**(VIC off) 단일 변경. softmax·Random학습·2M·seed42·100k×5 모두 동일. 러너 [`_run_softmax_novic_random.py`](../poker-pokerkit-prev/_run_softmax_novic_random.py).

| 탐색 | VIC | 런 | vsRand(=ID) | vsRule(=OOD) | 판정 |
|---|---|---|---|---|---|
| Softmax | **on** | 19 | +277.0 | **+112.1** | BOTH+ |
| Softmax | **off** | softmax_novic_random_2m | **+760.4** (40.1) | **−27.2** (22.6) | OOD 브레이크이븐(−), ID↑ |
...
- [`ablation_vic_2m/`](ablation_vic_2m/) — VIC ablation(28번) 본 실험 6개 런 + `_summary_100kx5.csv`
- [`pure_softmax_mixed_2m/`](pure_softmax_mixed_2m/) — Pure-vs-Prop(29번) PURE 런
- [`_reeval_100kx5/`](_reeval_100kx5/) — 16~26번 100k×5 통일 재평가 로그 + `_summary_reeval_100kx5.csv` + `aggregate.py`
- [`softmax_novic_random_2m/`](softmax_novic_random_2m/) — (Softmax × VIC) 2×2 통제. 19번과 100% 매칭, VIC만 off. 메인 라인에서 "softmax≠VIC 대체" 입증
