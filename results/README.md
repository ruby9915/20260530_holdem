...
|---|---|---|---|---|---|---|---|
| 1 | [`01_mc_eps_imm_40k`](01_mc_eps_imm_40k/) | MC | ε-greedy | terminal + imm | 40k | (기준선) | 정책 붕괴 (vs Rand 32%) |
...
> total_invest==0 균등분배 예외 처리(누수)가 원인 — 누수 격리런(LEAKON)이 원본 −1261.8/+791.2를 완전
...
|---|---|---|
| off(α=0) | −318±123 | 0/6 |
| **fixed 1칩 (E7, 2026-07-06)** | **−117±252** | **2/6 — 전이구간(부호 뒤바뀜)** |
...
> **야간 체인 E1~E6 결과(실험일지 33절)로 32절 미해결 4건 종결:** ① 올체크 격리(E1) → 효과 본체=CHECK credit ② 홀드아웃(E2) → "일반화" 기각·상대 의존(STA −177) ③ fixed-K(E3) → 상수 ε도 임계만 넘으면 유효(hindsight 완전 소거) ④ 메커니즘(E4) → 턴 체크 65%→소액 벳 65% 소극화 해소.
...
## 대안 해법 bake-off (= 31번, E8, 2026-07-06)

[`31_rival_bakeoff/`](31_rival_bakeoff/) — ZCA에 대한 통상적 해법 후보 3종을 동일 규약(clean·single-TAG·2M·100k×5)으로 통제 비교. 러너 [`_run_rival_seed.py`](../poker-pokerkit-prev/_run_rival_seed.py) (`UNIFORM_PENALTY` 훅 신설, 낙관 초기화·온도 하한은 monkey-patch).

| 해법 (×seed 1-5) | vsRand | 양수 | vsTAG | 판정 |
|---|---|---|---|---|
| `slowT2_s*` 탐색 강화(온도≥2.0) | −291±267 | 1/5 | +880±25 | 무효(재고정) — 예측 적중 |
| `opt50_s*` 낙관적 초기화(Q₀=50) | −241±302 | 1/5 | **+184±43 훼손** | 무효 + ID 훼손 |
| `unif5_s*` 일률 벌점(전 행동 −5칩) | −485±32 | 0/5 | **+119±34 훼손** | 무효 + 양 지표 악화(예측 편차 — 미방문 0과 상대 격차) |
| (대조) 선택적 5칩 `fixed_k5_s*` | +1230±693 | 5/5 | +909±50 | **유일 성공** |

> 논문 §5.5 신설. "비용 부여" 일반이 아니라 **비용 0 행동에 대한 선택성**이 해법의 본질임을 ③과의 대비로 분리 실증. 상세 실험일지 33절 보강 3.
...
[`34_threshold_stage0/`](34_threshold_stage0/) — 신규 학습 없이 기존 Q-테이블(6플랫폼)·앵커 71건으로 VIC 임계의 무차원 자기일관성(P1)·probit 이식(M1/M2)을 검증. 핵심: 구판 오염도 0(E9 수치 무사), α_min 무차원 수축(칩 3.5배→α 2.1배, 단 A8/A12 구조 잔존), M2 out-of-sample 9/10 적중(유일 실패 = k20-A12 fixed5 — 게이트 A의 결정 시험으로 승격), **게이트 A/B 사전 등록**(폴더 README 표). 도구 [`analyze_threshold.py`](../poker-ladder/analyze_threshold.py) v2 + [`fit_probit_stage0.py`](../poker-ladder/fit_probit_stage0.py). 상세 실험일지 35절 보강 20.
...
**결과(2026-08-25 완주)**: P1 ✓ off 5시드 전부 Q(CHECK) 정확히 0(지문 — ZCA는 탐색 무관 구조 성질) / P2 ✓ off vsRand −229±37(0/5) 붕괴 재현 / P4 ✓ vsTAG 양 조건 +930·+946 보존 / **P3 ✗ fixed5 vsRand −269±200(1/5) — 회복 실패, 사전 등록 기각 기준 발동**: 처방의 성능 효과는 softmax 탐색과의 결합에 한정될 가능성(Q(CHECK)는 풀렸으나 성능 미회복 — 흡수 해소≠성능 회복의 새 사례). 부수: ε-greedy 활동 셀 189~191(협소), off의 CHECK-argmax 0%(붕괴 경로가 softmax판과 다를 가능성 — 해부는 게이트). 상세 = 실험일지 36절 보강 1.
...
**결과(2026-08-26 완주)**: **등록 예측 기각** — pure는 vsCFR −621→−605로 1M에서 이미 평탄(결손은 속도가 아니라 점근선; 컴퓨팅 비용 논거 사용 금지). 성립하는 대체 진술 = 격자 전 지점(예산 5점×3축)에서 chec_a30 ≥ pure. **예측 밖 발견(게이트 카드)**: chec_a30이 예산 증가에 vsCFR −51→−484·vsRand +2328→+309 단조 악화, vsTAG는 개선 — 1M 저예산이 오히려 최상(vsCFR 2/5 양수, 전 격자 유일). 기전 해부는 별도 사전 등록 후. 상세 = 실험일지 36절 보강 3.
...
- **1번** — 기준선. MC + ε-decay + imm 조합이 학습 초반(약 10k ep) 정점을 찍은 뒤 무너지는 것을 처음 관측. "성능이 오르다 떨어진다"는 현상 자체를 데이터로 고정해, 이후 모든 실험의 비교 기준점이 된다.
...
- **16번** — CHECK=1chip(VIC) 추가. 기존엔 "vs Rule mbb **+10** 첫 양수 전환"으로 흡수 고정점 해법의 역증명으로 썼으나, **100k×5 정밀평가에서 vsRule −1,145.8(SD 28.2)로 강한 적자**임이 드러나 이 서사는 무효(+10은 200게임 체크포인트 착시). 16은 VIC-on **단독** ε-greedy/1.2M 런으로 vsRand +4,981.6 / vsRule −1,145.8 = **OOD+/ID−**의 다른 양식이며, ZCA/VIC 인과는 ablation(28번)에만 귀속한다. 16은 'VIC 원형의 정성 사례'로만 분리 표기.
...
- [`_pure_smoke/`](_pure_smoke/) — 29번 Pure-vs-Prop 착수 전 PURE 스모크(사전 검증)
- [`28c_seedsweep/`](28c_seedsweep/) — **학습 seed sweep** {single/cycle/mixed × on/off} × seed{1–5} + Random 라인 ±VIC×5 (2M·100k×1, **누수 정정 이전 코드**). 28번의 "VIC=OOD 필요조건" 주장이 seed=42 가공물임을 밝힌 근거(단일 seed 오도 사례 — 논문 VI장·실험일지 32절). 수치는 성능 권위값 아님, **방법론적 증거로 보존**
